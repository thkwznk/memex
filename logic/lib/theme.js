...
function Theme(
  ...
  this.start = function () {
    const storage = is_json(localStorage.theme)
  ...
  this.parse = function (any) {
    if (any && any.background) {
      ...
      return any.data;
    } else if (any && is_json(any)) {
      return JSON.parse(any);
    } else if (any && is_html(any)) {
      return this.extract(any);
    }
  ...
  this.invert = function () {
    this.load(
      this.active.background == this.collection.noir.background
        ? this.collection.pale
        : this.collection.noir
    );
  };
